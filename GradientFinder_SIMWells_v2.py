...
obsQ = pd.read_excel(xls,'WaterQuality', header = [0,1], parse_dates=True, infer_datetime_format=True, dayfirst=True)
...
# Filter outliers
#for name, group in obsH_.groupby(by = 'WellID'):
#    print(name)
...
obsH_['year'] = obsH_['DT_Days'].astype('datetime64[Y]')
for i, year in obsH_.groupby('year'):
    fig, ax = plt.subplots()
...
for comb in combs:
    ...
    coefs = []
    flows = []
    for name, group in obsH_.groupby(['DT_Days']):
