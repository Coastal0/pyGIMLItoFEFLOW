...
def findMeshes():
    ...
    vec = pg.load('resistivity.vector')
    for m in meshes:
        temp_mesh = pg.load(m)
        if vec.size() == temp_mesh.cellCount():
            return temp_mesh
            break
...
def showVec(lastOnly, cmap):
    plt.close('all')
    meshInv = findMeshes()
    print("No# MeshCells =", meshInv.cellCount())
    cov = pg.load("coverage.vector")
    ...
    #        ax = plt.gca()
            ...
            ax[vecName[0]].set_xlim(left=-50, right=600)
